<div>
    {{-- The best athlete wants his opponent at his best. --}}

    <div class="flex flex-col items-center justify-center min-h-screen gap-6">
        <h1 class="text-3xl font-bold mb-4">welcome to dashboard</h1>
        <nav class="flex gap-4">
            <a href="/books" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Books</a>
            <a href="/customers" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Customers</a>
            <a href="/borrowings" class="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700">Borrowings</a>
        </nav>
    </div>
